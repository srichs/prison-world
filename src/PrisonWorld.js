/**
 * Filename: PrisonWorld.js
 * Date: 4 Mar 2020 <p>
 *
 * Purpose: This javascript file creates a 3D graphics scene using the WebGL
 *     library.
 *
 * @author srichs
 */
...
var prisonLightsOn = true;
var emergencyLightsOn = true;

/** Draws the image, which consists of the world. */
function draw() {
...
/** This function handles the lights of the scene. */
function lights() {
    if (daytime) { // light 1 is the sun
        gl.uniform3f(u_lights[1].color, 0.7, 0.7, 0.4); // light 1 is the sun during the day
        gl.uniform1f(u_lights[1].attenuation, 0);
    } else {
        gl.uniform3f(u_lights[1].color, 1, 1, 0.8); // light 1 is the moon at night
    ...
    mat4.translate(modelview, modelview, [12, 6, 0]);
    mat4.scale(modelview, modelview, [0.4, 0.4, 0.4]);
    if (daytime) {
        gl.uniform3f(u_material.emissiveColor, 0.7, 0.7, 0.1);
    ...
    pushMatrix(); // draw the moon, with emissive color at night
    mat4.translate(modelview, modelview, [6, 12, 0]);
    mat4.scale(modelview, modelview, [0.4, 0.4, 0.4]);
    if (!daytime) {
        setLightPosition(u_lights[1].position, modelview, [4, 2, 0, 1]);
        gl.uniform3f(u_material.emissiveColor, 0.7, 0.7, 1);
    }
    sphere.render();
    gl.uniform3f(u_material.emissiveColor, 0, 0, 0);
    popMatrix();
    ...
    if (!headLightsOn) {
        ...
        mat4.translate(modelview, modelview, [0, 0.3, 10]);
    ...
    if (!prisonLightsOn) {
        ...
        pushMatrix();
        mat4.translate(modelview, modelview, [5, 6, 10]);
        mat4.rotate(modelview, modelview, (90) / 180 * Math.PI, [6, 0, 4]);
        mat4.scale(modelview, modelview, [0.3, 0.3, .3]);
        pushMatrix();
        mat4.translate(modelview, modelview, [-3, 10, -1]);
        setLightPosition(u_lights[4].position, modelview, [0, 0, 0, 1]);
        setSpotlightDirection(u_lights[4].spotDirection, modelview, [-4, 0, 0]);
        popMatrix();
        pushMatrix();
        mat4.translate(modelview, modelview, [-3, 0.6, 1]);
        mat4.rotateY(modelview, modelview, -Math.PI / 12);
        ...
        pushMatrix();
        mat4.translate(modelview, modelview, [3, 10, 1]);
        setLightPosition(u_lights[6].position, modelview, [0, 0, 0, 1]);
        setSpotlightDirection(u_lights[6].spotDirection, modelview, [4, 0, 0]);
        popMatrix();
        pushMatrix();
        mat4.translate(modelview, modelview, [-3, 0.6, 1]);
        mat4.rotateY(modelview, modelview, -Math.PI / 12);
        setLightPosition(u_lights[7].position, modelview, [0, 0, 0, 1]);
        setSpotlightDirection(u_lights[7].spotDirection, modelview, [-1, 0, 0]);
        popMatrix();
        popMatrix();
    }
}
 ...
 */
function world() {
    pushMatrix(); // the disk that the world is on
    ...
    pushMatrix();
    mat4.translate(modelview, modelview, [3.5, 3, 5]);
    ...
    pushMatrix(); // creates perimeter road
    currentColor = [0.3, 0.3, 0.3, 1];
    mat4.rotate(modelview, modelview, (90) / 180 * Math.PI, [-1, 0, 0]);
    ring.render();
    popMatrix();

    pushMatrix(); // creates the patrol car
    mat4.rotate(modelview, modelview, (-frameNumber) / 180 * Math.PI, [0, 1, 0]);
    mat4.translate(modelview, modelview, [0.3, 0.3, 10]);
    mat4.scale(modelview, modelview, [0.3, 0.3, .3]);
    patrolCar();
    popMatrix();
...
/** This function creates the trees for the scene. */
...
/** This function creates a guard tower. */
function guardTower() {
    pushMatrix(); // base of the tower
    mat4.scale(modelview, modelview, [1, 6, 1]);
    ...
    mat4.scale(modelview, modelview, [2, 3, 2]);
    currentColor = [1, 1, 1, 1];
    cube.render();
    popMatrix();
    ...
    if (prisonLightsOn) {
        gl.uniform3f(u_material.emissiveColor, 0.4, 0.4, 0.4);
    }
    pushMatrix();
    mat4.translate(modelview, modelview, [1, 4, 0]);
    mat4.scale(modelview, modelview, [0.1, 0.25, 0.25]);
    sphere.render();
    popMatrix();
    pushMatrix();
    mat4.translate(modelview, modelview, [0, 4, 1]);
    mat4.rotate(modelview, modelview, (90) / 180 * Math.PI, [0, 1, 0])
    mat4.scale(modelview, modelview, [0.1, 0.25, 0.25]);
    sphere.render();
    popMatrix();
...
/** This function creates a prison and walls. */
function prison() {
    pushMatrix(); // the prison
    ...
    pushMatrix(); // right side wall
    mat4.translate(modelview, modelview, [4.5, 0.75, 0]);
    mat4.rotate(modelview, modelview, (90) / 180 * Math.PI, [0, 0, 0]);
    mat4.scale(modelview, modelview, [0.25, 1.5, 12]);
    currentColor = [1, 1, 1, 1];
    cube.render();
    popMatrix();

    pushMatrix(); // left side wall
    mat4.translate(modelview, modelview, [-4.5, 0.75, 0]);
    mat4.rotate(modelview, modelview, (90) / 180 * Math.PI, [0, 0, 0]);
    mat4.scale(modelview, modelview, [0.25, 1.5, 12]);
    currentColor = [1, 1, 1, 1];
    ...
    pushMatrix(); // rear wall
    mat4.translate(modelview, modelview, [0, 0.75, -6]);
    mat4.rotate(modelview, modelview, (90) / 180 * Math.PI, [0, 1, 0]);
    mat4.scale(modelview, modelview, [0.25, 1.5, 9.25]);
    currentColor = [1, 1, 1, 1];
    cube.render();
    popMatrix();

    pushMatrix(); // front left wall
    mat4.translate(modelview, modelview, [-2.625, 0.75, 6]);
    mat4.rotate(modelview, modelview, (90) / 180 * Math.PI, [0, 1, 0]);
    mat4.scale(modelview, modelview, [0.25, 1.5, 4]);
    currentColor = [1, 1, 1, 1];
    cube.render();
    popMatrix();

    pushMatrix(); // front right wall
    mat4.translate(modelview, modelview, [2.625, 0.75, 6]);
    mat4.rotate(modelview, modelview, (90) / 180 * Math.PI, [0, 1, 0]);
    mat4.scale(modelview, modelview, [0.25, 1.5, 4]);
    currentColor = [1, 1, 1, 1];
    ...
    pushMatrix(); // front walkway left wall
    mat4.translate(modelview, modelview, [-0.5, 0.75, 4.125]);
    mat4.rotate(modelview, modelview, (90) / 180 * Math.PI, [0, 0, 0]);
    mat4.scale(modelview, modelview, [0.25, 1.5, 4]);
    currentColor = [1, 1, 1, 1];
    cube.render();
    ...
    pushMatrix(); // front walkway right wall
    mat4.translate(modelview, modelview, [0.5, 0.75, 4.125]);
    mat4.rotate(modelview, modelview, (90) / 180 * Math.PI, [0, 0, 0]);
    mat4.scale(modelview, modelview, [0.25, 1.5, 4]);
    currentColor = [1, 1, 1, 1];
    ...
    pushMatrix(); // prison door
    mat4.translate(modelview, modelview, [0, 1.25, 2.5]);
    mat4.rotate(modelview, modelview, (90) / 180 * Math.PI, [0, 1, 0]);
    mat4.scale(modelview, modelview, [0.25, 2.5, 0.75]);
...
/**
 * Draws a car consisting of two scaled blue cubes with headlights
...
/**
 *  Draw an axle that consists of a long yellow cylinder with
 *  a wheel on each end.
 */
function axle() {
...
function initGL() {
    ...
    gl.uniform1f(u_lights[4].spotCosineCutoff, Math.cos(Math.PI / 8));
    ...
    gl.uniform1f(u_lights[6].spotCosineCutoff, Math.cos(Math.PI / 8));
    gl.uniform1f(u_lights[7].spotCosineCutoff, Math.cos(Math.PI / 8)); //    which are spotlights
    gl.uniform3f(u_lights[6].color, 0.5, 0.5, 0.4);
    gl.uniform3f(u_lights[7].color, 0.5, 0.5, 0.4);
...
function doSlideChange() {
    frameSpeed = document.getElementById("range1").value * 0.01;
}

/** This function turns on and off the headlights. */
...
/** This function turns on and off the prison spotlights. */
function changePrisonLights() {
    if (prisonLightsOn) {
        prisonLightsOn = false;
    } else {
        prisonLightsOn = true;
    }
}

/** This function turns on and off the prison emergency lights. */
function changeEmergencyLights() {
    if (emergencyLightsOn) {
...
function init() {
    ...
    document.getElementById("r1").onclick = function() {changeTOD(0);};
    document.getElementById("r2").onclick = function() {changeTOD(1);};
    document.getElementById("light1").checked = headLightsOn;
    document.getElementById("light1").onchange = function() {changeHeadLights();};
    document.getElementById("light2").checked = prisonLightsOn;
    document.getElementById("light2").onchange = function() {changePrisonLights();};
    document.getElementById("light3").checked = emergencyLightsOn;
    document.getElementById("light3").onchange = function() {changeEmergencyLights();};
    document.getElementById("range1").oninput = function() {doSlideChange();};
    ...
    ring = createModel(ring(9.0, 11, 40));
